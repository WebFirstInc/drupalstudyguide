...
Twig uses the `{{ }}` syntax to output data in templates.
```
::::
::: code-group
```[Question 2]
Which of the following statements about accessing arrays 
and objects in Twig is true?

1. You can only access array items using the subscript syntax [].
2. The dot syntax foo.bar can be used for both arrays and objects.
3. The dot syntax foo.bar only works with PHP arrays.
4. You cannot access object methods using Twig syntax.
```

```[Answer]
2. The dot syntax foo.bar can be used for both arrays and objects.

This allows themers to not worry about variable types, and just 
output the data. Array items can also be accessed using the 
subscript syntax ([]):
```
::::
::: code-group
...
1. To transform a variable
2. To execute a conditional statement
3. To translate text using tokens
4. To create a loop
```
...
The Twig {% trans %} block will translate the text using tokens 
with t() or format_plural() if the {% plural ... %} switch has 
been declared in the tag
```
::::
