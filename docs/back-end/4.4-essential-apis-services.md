...
<Question title='How to use Entity Type Manager service to load an entity by condition field_test = "test" and content type is "article"?'>
...
**Note:** `loadByProperties` bypasses access checks [EntityStorageBase::loadByProperties](https://git.drupalcode.org/project/drupal/-/blob/10.3.x/core/lib/Drupal/Core/Entity/EntityStorageBase.php?ref_type=heads#L605). If you need access checks, use `getQuery()` instead.
</Question>

<Question title='How can you alter an existing service in Drupal?'>
