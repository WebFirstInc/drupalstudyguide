...
<Question title='You have defined a custom route with the URL "/test-url/test". What will happen if User tries to access "/test-url///test"?'>

There will be no error. User will be redirected to "test-url/test" automatically.
</Question>

<Question title='You need to have a new URL like "/node/{node}/test". How can you pass the entire node object to the controller?'>
