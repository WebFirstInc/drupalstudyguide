...
<details>
<summary>How do you validate a form created with Form API?</summary>

In `validateForm`, you use FormState's `setErrorByName` function, like this:
```php
public function validateForm(array &$form, FormStateInterface $form_state) {
  $form_state->setErrorByName('age', $this->t('You must be 13 or older to use this site.'));
}
...
<details>
<summary>How would you add additional validation to an existing form?</summary>

You can use `hook_form_alter` or `hook_form_FORM_ID_alter` to do this, by adding your function to the array of validations:

```php
function example_form_existing_form_alter(array &$form, FormStateInterface &$form_state, $form_id) {
  $form['#validate'][] = '_example_existing_form_custom_validation';
}

function _example_existing_form_custom_validation(array &$form, FormStateInterface &$form_state, $form_id) {
...
<details>
<summary>What two things are required to render a form at a specific URL?</summary>

1. The form class inheriting from `Drupal\Core\Form\FormBase` (or other ancestor).
2. A routing entry with the `_form` property.

See [Form Routing](#form-routing) above.
</details>
